  ...
  private async processScreenshotsHelper(
    ...
    try {
      ...
      if (config.apiProvider === "openai") {
        ...
        const messages = [
          {
            ...
            content:
              "You are a problem analyzer. First, determine if this is a multiple choice question (MCQ) or a coding problem. If it's an MCQ, respond ONLY with the correct answer option (A, B, C, D, etc.). If it's a coding problem, return structured JSON with fields: problem_statement, constraints, example_input, example_output. Just return the answer or JSON without any other text.",
          },
          {
            role: "user" as const,
            content: [
              {
                type: "text" as const,
                text: `Analyze these screenshots. If it's a multiple choice question, respond with just the correct answer option. If it's a coding problem, extract the details in JSON format. Preferred coding language is ${language}.`,
              },
        ...
        if (responseText && /^[A-Z]$/.test(responseText.trim())) {
          isMCQ = true;
          problemInfo = {
            problem_statement: "Multiple Choice Question",
            mcq_answer: responseText.trim(),
            constraints: "N/A",
            example_input: "N/A",
            example_output: responseText.trim(),
          };
        } else {
          try {
          ...
          const geminiMessages: GeminiMessage[] = [
            {
              ...
              parts: [
                {
                  text: `Analyze the screenshots. First determine if this is a multiple choice question (MCQ) or a coding problem.

If it's a multiple choice question (MCQ), respond ONLY with the correct answer option (A, B, C, D, etc.)

If it's a coding problem, return the information in JSON format with these fields: problem_statement, constraints, example_input, example_output. Just return the structured JSON without any other text.
...
Preferred coding language is ${language}.`,
          ...
          if (/^[A-Z]$/.test(cleanedResponseText)) {
            isMCQ = true;
            problemInfo = {
              problem_statement: "Multiple Choice Question",
              mcq_answer: cleanedResponseText,
              constraints: "N/A",
              example_input: "N/A",
              example_output: cleanedResponseText,
            };
          } else if (
            cleanedResponseText.includes("multiple choice") &&
            /\b[A-Z]\b/.test(cleanedResponseText)
          ) {
            const answerMatch = cleanedResponseText.match(/\b([A-Z])\b/);
            if (answerMatch) {
              isMCQ = true;
              problemInfo = {
                problem_statement: "Multiple Choice Question",
                mcq_answer: answerMatch[1],
                constraints: "N/A",
                example_input: "N/A",
                example_output: answerMatch[1],
              };
            }
          } else {
            try {
              ...
              problemInfo = {
                ...
                constraints: "See screenshot for details",
                example_input: "See screenshot for examples",
                example_output: "See screenshot for expected output",
              };
            }
          }
        } catch (error) {
          ...
          const messages = [
            {
              ...
              content: [
                {
                  type: "text" as const,
                  text: `Analyze these screenshots. First determine if this is a multiple choice question (MCQ) or a coding problem. If it's an MCQ, respond ONLY with the correct answer option (A, B, C, D, etc.). If it's a coding problem, return structured JSON with fields: problem_statement, constraints, example_input, example_output. Preferred coding language is ${language}.`,
                },
                ...imageDataList.map((data) => ({
                  type: "image" as const,
                  source: {
      ...
      if (mainWindow) {
        ...
        if (isMCQ) {
          const mcqResult = {
            code: `Answer: ${problemInfo.mcq_answer}`,
            thoughts: [
              "This is a multiple choice question",
              `The correct answer is: ${problemInfo.mcq_answer}`,
            ],
            time_complexity: "N/A - MCQ",
            space_complexity: "N/A - MCQ",
            is_mcq: true,
          };
          ...
          if (mainWindow) {
            mainWindow.webContents.send("processing-status", {
              message: "MCQ answer identified successfully",
              progress: 100,
            });
          }
